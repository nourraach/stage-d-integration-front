...
import { Location, Photo, ApiResponse } from '../shared/models/location.model';
...
toggleFavorite(id: number, favorite: boolean): Observable<any> {
  return this.http.put(`${this.apiUrl}/${id}/favorite`, { favorite });
}

getFavorites(): Observable<Location[]> {
  return this.http.get<{data: Location[]}>(`${this.apiUrl}/favorites`).pipe(
